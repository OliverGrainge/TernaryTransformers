...
class DataConfig:
    ...
    checkpoints_dir: str = "/home/oliver/Documents/github/TernaryTransformers/examples/checkpoints"

    @classmethod
    def add_args(cls, parser: argparse.ArgumentParser):
        # Add all arguments without separate grouping
        for field in cls.__dataclass_fields__.values():
            parser.add_argument(f"--{field.name}", type=field.type, default=getattr(cls, field.name),
                              help=f"{field.name} parameter")

    @classmethod
    def from_args(cls, args):
        return cls(**{field.name: getattr(args, field.name) 
                     for field in cls.__dataclass_fields__.values()})
...
def parse_configs(ModelConfig, TrainConfig, DataConfig): 
    ...
    return ModelConfig.from_args(args), TrainConfig.from_args(args), DataConfig.from_args(args)


if __name__ == "__main__":
    model_config, training_config, data_config = parse_configs(ModelConfig, TrainConfig, DataConfig)
    print(model_config)
